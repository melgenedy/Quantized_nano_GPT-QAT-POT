...
exec(open(file='config/train_shakespeare_char_qat.py').read()) #exec(open('configurator.py').read()) # overrides from command line or config file
...
def get_nanoGPT_quantization_config(
    ...
    act_qmin: int = -128,#-2**31,#-128,
    act_qmax: int = 127,#2**31-1,#127,
    weight_qmin: int = -8,#-127,#-15,#-2**31+1,#-127,
    weight_qmax: int = 4,#127#15,#2**31-1,#127,
):
...
#quantizedquantizer2 = XNNPACKQuantizer().set_global(get_symmetric_quantization_config(is_qat=True))
